...
class AppointmentListCreate(APIView):
    ...
    def post(self, request):
        ...
        patient_service_url = f'http://127.0.0.1:8000/api/patients/{
            patient_id}/'
        ...
        work_schedules_url = f'http://127.0.0.1:8003/api/doctors/{
            doctor_id}/work-schedules/'
        ...
        appointment_datetime_str = f"{
            appointment_date_str} {appointment_time_str}"
        appointment_datetime = datetime.strptime(
            appointment_datetime_str, '%Y-%m-%d %H:%M:%S')
        ...
        for schedule in work_schedules:
            if schedule['day_of_week'] == appointment_day_of_week:
                start_time = datetime.strptime(
                    schedule['start_time'], '%H:%M:%S').time()
                end_time = datetime.strptime(
                    schedule['end_time'], '%H:%M:%S').time()
